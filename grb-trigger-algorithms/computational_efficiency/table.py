...
if __name__ == "__main__":
    outputs = [
        "./outputs/results_focus.txt",
        "./outputs/results_gbm.txt"
    ]
    results = {
        'focus': {},
        'benchmark': {},
    ...
    for file, key in zip(outputs, results):
        with open(file) as f:
            ...
            for input_file, result in zip(lines[::2], lines[1::2]):
                ...
                results[key].setdefault((data_num, data_bkgrate), []).append(float(result.split(" s.")[0]))

    df_focus = pd.DataFrame(results["focus"]).stack().reset_index(level=0, drop=True).transpose().groupby(level=0, axis=1).mean()
    df_benchmark = pd.DataFrame(results["benchmark"]).stack().reset_index(level=0, drop=True).transpose().groupby(level=0, axis=1).mean()
