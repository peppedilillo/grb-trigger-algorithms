...
def make_count_matrix(cs):
    ...
    out = np.array([[sum(cs[col - row:col + 1]) if col >= row else np.nan for col in ids] for row in ids])
    return out


def make_snr_matrix(mc, b_rate):
    ids = np.arange(len(mc))
    out = np.array([[snr(mc[row, col], b_rate * (row + 1)) if col >= row else np.nan for col in ids] for row in ids])
    return out


def make_max_matrix(ms):
    ids = np.arange(len(ms))
    out = np.array([[ms[row, col]
                     if (ms[row, col] == max(ms[:, col]) and ms[row, col] > 0) else np.nan
                     # if ms[row,col] == max(ms[:,col]) else np.nan
                     for col in ids] for row in ids])
    return out
...
def make_gbm_matrix(ids, cs, b_rate, params, offsets=False):
    ...
    if offsets:
        out = np.array([[snr(mc[row, col], b_rate * (row + 1))
                         if (col >= row
                             and (((col + 1) % (row + 1) == 0)
                                  or (2 * (col + 1) % (row + 1) == 0))
                             and (row + 1) in params)
                         else np.nan for col in ids] for row in ids])
    else:
        out = np.array([[snr(mc[row, col], b_rate * (row + 1))
                         if (col >= row
                             and (col + 1) % ((row + 1)) == 0) and (row + 1) in params
                         else np.nan for col in ids] for row in ids])
    return out
...
def make_focus_curve_matrix(f_mem, ms):
    ...
    out = np.array([[ms[row, col]
                     if ((row, col) in tiles or (row == 0 and col <= max_col)) else np.nan
                     for col in ids] for row in ids])
    return out
...
def make_focus_max_matrix(f_mem, ms):
    ...
    max_col = max(list(zip(*tiles))[1])
    out = np.array([[ms[row, col]
                     if ((row, col) in tiles) else np.nan for col in ids] for row in ids])
    return out
...
def intersect(m1, m2):
    ...
    out = np.array([[m1[row, col]
                     if not np.isnan(m2[row, col]) else np.nan
                     for col in ids] for row in ids])
    return out


def plot(mp, ids, cs, b_rate, transient_time, transient_len, maxima_matrix=None, print_significance=True):
    """
        ...
        for (i, j), z in np.ndenumerate(maxima_matrix):
            if not np.isnan(z) and z >= 0:
                ...
                ax0.text(
                    ...
                    '{:0.1f}'.format(z),
                    size=font_size, ha='center', va='center', c=text_color, family='monospace',
                )
            elif z < 0:
                ax0.text(
                    j + 0.5,
                    i + 0.5 + vertical_offset,
                    ' '.format(0),
                    size=font_size, ha='center', va='center', c='white', family='monospace',
                )
    ax0.hlines(
        y=ids,
        xmin=ids - 1.,
    ...
    fig.colorbar(scores_all, orientation='horizontal', aspect=100, pad=0.01, label='Significance [$\sigma$]')
